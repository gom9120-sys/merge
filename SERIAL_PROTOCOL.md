...
- `us_*`는 20–1000mm, **0은 측정 실패**(에코 없음/범위 밖)다. 거리 0m이
  ...
  설치 높이/각도로 정해지는 값(기본 설정에서 약 150mm)이 나온다.
...
E,US_BEAM,41,26         빔이 노면에 그리는 띠 41mm, 전방 주시 26mm
...
홈은 5cm 근처가 한계다(초음파 빔이 노면에 41mm 띠를 그리므로 그보다
좁은 홈은 보이지 않는다). 그래서 "홈이 없다"가 아니라 "감지된 위험이

...
- `us_*`는 20–1000mm, **0은 측정 실패**(에코 없음/범위 밖)다. 거리 0m이
  ...
  설치 높이/각도로 정해지는 값(기본 설정 16cm·85도에서 약 161mm)이 나온다.
...
E,US_BEAM,42,14         빔이 노면에 그리는 띠 42mm, 전방 주시 14mm
E,US_VMAX,46            홈 너비 판정이 유효한 한계 속도 46cm/s
...
홈은 5cm 근처가 한계다(초음파 빔이 노면에 42mm 띠를 그리므로 그보다
좁은 홈은 보이지 않는다). 각도로 얻는 전방 주시가 14mm뿐이라 경보를
얼마나 미리 받을지는 센서를 바퀴보다 얼마나 앞에 다는지가 정한다. 그래서 "홈이 없다"가 아니라 "감지된 위험이
